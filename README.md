# [KRAngular](http://github.com/krainet/KRAngular) 
...
```sh
$ git clone git://github.com/krainet/KRAngular
$ cd KRAngular
...
The principal goal of `KRAngular` is to set projects up for long-term
success.  So `KRAngular` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `KRAngular`.

But `KRAngular` is not an example of an AngularJS app: this is a
...
getting `KRAngular` up and running. Though `KRAngular` is really simple
...
`KRAngular` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `KRAngular`, like Twitter Bootstrap
...
Technically, `KRAngular` is now ready to go.
...
`KRAngular`. What follows in this section is a quick introduction to the
...
`KRAngular` also includes [Live Reload](http://livereload.com/), so you no
...
default with `KRAngular`) you need to tell Live Reload to allow it. Go to
